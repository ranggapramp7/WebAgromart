...
<div class="container">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php 
            $result = mysqli_query($conn, "SELECT DISTINCT invoice, kode_customer, status, qty, terima, tolak, cek FROM produksi GROUP BY invoice");
            $no = 1;
                ...
                <tr>
                    ...
                    <td>
                        <?php if ($row['tolak'] == 0 && $row['cek'] == 1 && $row['terima'] == 0) { ?>
                            <a href="inventory.php?cek=0" id="rq" class="btn btn-warning"><i class="glyphicon glyphicon-warning-sign"></i> Ketersediaan barang</a> 
                        ...
                        <?php } else if ($row['terima'] == 0 && $row['cek'] == 0) { ?>
                            ...
                            <a href="proses/tolak.php?inv=<?= $row['invoice']; ?>" class="btn btn-danger" onclick="return confirm('Yakin Ingin Menolak?')"><i class="glyphicon glyphicon-remove-sign"></i> Tolak</a>
                        <?php } ?>
                        <a href="detailorder.php?inv=<?= $row['invoice']; ?>&cs=<?= $row['kode_customer']; ?>" class="btn btn-primary"><i class="glyphicon glyphicon-eye-open"></i> Detail Pesanan</a>
                    </td>
                </tr>
            <?php
                $no++; 
